...
class FacultyController extends Controller
{
    ...
    public function viewClass(Request $request){
        $details = FacultyStaff::select('*', 'faculty_staff_personal_details.id as fid', 'faculty_staff_personal_details.firstname as ffname', 'faculty_staff_personal_details.lastname as flname','subject.grade_level AS class_glevel', 'class.id as cid' , DB::raw('COUNT(student_personal_details.id) AS ctr'))->join('faculty_staff_user_mapping', 'faculty_staff_user_mapping.faculty_staff_id', '=', 'faculty_staff_personal_details.id')->join('users', 'users.id', '=', 'faculty_staff_user_mapping.user_id')->join('user_roles', 'user_roles.id', '=', 'users.role')->join('class', 'class.faculty_id', '=', 'faculty_staff_personal_details.id')->join('student_subject', 'student_subject.id', '=', 'class.student_subject_id')->join('subject', 'subject.id', '=', 'student_subject.subject_id')->join('student_personal_details', 'student_personal_details.id', '=', 'student_subject.student_id')->where('class.id', $request->id)->first();
        $classes = DB::table('class')->select('*', 'class.id as cid','subject.grade_level AS class_glevel')->join('student_subject', 'student_subject.id', '=', 'class.student_subject_id')->join('subject', 'subject.id', '=', 'student_subject.subject_id')->join('student_personal_details', 'student_personal_details.id', '=', 'student_subject.student_id')->join('school_year', 'school_year.id', '=', 'class.sy_id')->where('class.id', $request->id)->groupBy('class.student_subject_id')->get();
